  ...
  return (
    <div
      className={`mainHeader ${
        menu ? "open" : ""
      } bg-white lg:fixed cursor-default top-0 fixed lg:z-[1111111111] z-[1111111] min-h-[80px] lg:min-h-[90px] py-[1rem] md:px-[2.5rem] px-[1rem] flex lg:flex-row flex-col lg:gap-0 gap-10 items-start lg:items-center md:justify-between w-full`}
    >
      <div>
        <Link href="/">
          <Image
      ...
      <div
        className={`menu-slide ${
          menu ? "open" : ""
        } flex lg:flex-row flex-col gap-6 lg:gap-[2rem] items-start lg:items-center h-full justify-center`}
      >
        {Nav_links.map((link) => (
          <div className="relative" key={link.id}>
            <Link
              ...
              className={`${ManropeFont} text-xl text-black`}
              href={link.url}
            >
              {link.label}
            </Link>
      ...
      <div className={`hero_atc ${menu ? "show" : ""} flex`}>
        <Link
          ...
          className={` ${ManropeFont} transition-all ease-in-out bg-black px-[2rem] py-[.7rem] max-h-[3rem] h-[3rem] text-white rounded-full items-center justify-center `}
          href="/contact"
        >
          Contact Us
        </Link>
