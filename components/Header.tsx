...
const Header = () => {
  ...
  return (
    <div className={`mainHeader ${menu ? 'open' : ''} bg-white lg:fixed cursor-default top-0 fixed lg:z-[1111111111] z-[1111111] min-h-[80px] lg:min-h-[90px] py-[1rem] md:px-[2.5rem] px-[1rem] flex lg:flex-row flex-col lg:gap-0 gap-10 items-start lg:items-center md:justify-between w-full`}>
      <div>
        <Link href="/">
          <Image className='w-auto h-[45px] lg:h-[55px]' src={logoUrl} width={180} height={75} alt='Node Logo' />
        </Link>
      </div>
      <div className={`menu-slide ${menu ? 'open' : ''} flex lg:flex-row flex-col gap-6 lg:gap-[2rem] items-start lg:items-center h-full justify-center`}>
        {Nav_links.map(link => (
      ...
      <div className={`hero_atc ${menu ? 'show' : ''} flex`}>
        <Link onClick={() => { setMenu(prev => !prev); }} className='transition-all ease-in-out bg-black px-[2rem] py-[.7rem] max-h-[3rem] h-[3rem] text-white rounded-full items-center justify-center' href='/contact'>
          Contact Us
        </Link>
      ...
      <div className='absolute lg:hidden flex items-center right-[1rem] top-[1.5rem]'>
        <svg onClick={toggleMenu} className={`hamburger-icon ${menu ? 'open' : ''}`} width="33px" height="33px" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" fill="none">
          {menu ? (
