...
const Services: React.FC = () => {
  ...
  return (
    <div style={{ scrollMarginTop: '50px' }} id="services" className='z-[11] relative flex lg:flex-row flex-col items-center pt-[50px] md:py-[50px] justify-start bg-black'>
      {/* Left Column (Text) */}
            ...
            <SwiperSlide key={item.id} className='bg-[#1A1C21] flex flex-col p-8 rounded-lg'>
              <div className='flex h-full flex-col items-start justify-between'>
                ...
                <div className='flex flex-col text-white text-start gap-4'>
                  ...
                  className={`text-2xl text-orange-500 lg:text-3xl font-bold ${Rancher}`} style={{ transition: 'opacity 0.3s ease-in-out',
                     lineHeight: "120%"
                   }}>
                    {item.title}
                  </h3>
                  <h5 className={`text-sm lg:text-lg ${SedaN}`} style={{ transition: 'opacity 0.3s ease-in-out' }}>
                    {item.subtitle}
                  </h5>
                </div>
              </div>
